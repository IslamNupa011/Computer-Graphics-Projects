...
/*The Game over State*/
void draw_gameover_scene(char *text)
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glLoadIdentity();
    glTranslatef(float(dox), float(doy), -6.0f);
...
{
    if(over == 1){
        if(reset_count <= 1) draw_gameover_scene("Press S to start");
        else draw_gameover_scene("Game Over!! :(");
    }
    else{
        draw_mainscene();
    }
...
void update(int flag)
{
    ...
    glutTimerFunc(1000.0/currentFPS, update, 1);
...
void state_reset(int newStart, int newOver)
{
    int i;
